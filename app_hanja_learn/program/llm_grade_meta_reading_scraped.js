...
/**
 * LLM에 전송할 데이터만 추출 (id, kanji, examples만 포함)
...
/**
 * 데이터를 압축 형식으로 변환
 * 형식: id|kanji|emoji|onyomi_readings|kunyomi_readings|unknown_readings
 ...
 * 작업 로직 (각 줄을 단계별로 처리):
 * 1. 한자 뜻 풍부화:
 *    - 한자의 영어 뜻('en_meaning') 추가
 *    - 형식: 'id|kanji|en_meaning|...'
 *    - 뜻이 여러 가지라면 쉼표로 구분
 *
 * 2. 예시의 빈 뜻 채우기:
 ...
 *
 * 데이터 형식 규칙:
 * - 구조: 'id|kanji|en_meaning|onyomi_readings|kunyomi_readings|unknown_readings'
 ...
 * 예시:
 * - 입력:
 *   '2|哀|アイ[哀悼:애도:,哀愁:애수:,哀願:애원:,悲哀:비애:,哀愁::,哀願::,悲哀::]|あわ(れ)[];あわ(れむ)[]|あわれ[哀れ::,哀れな話::,哀れがる::];あわれむ[哀れむ::,哀れみ::]'
 *   '3|愛|アイ[愛国:애국:,愛人:애인:]||'
 * - 출력:
 *   '2|哀|pity, sorrow|*アイ[哀悼:애도:condolence,哀愁:애수:melancholy,哀願:애원:entreaty,悲哀:비애:sorrow,哀愁:애수:melancholy,哀願:애원:entreaty,悲哀:비애:sorrow]|*あわれ[哀れ:비애:pity,哀れな話:가련한 이야기:pity story,哀れがる:가엾이 여기다:feel pity];あわれむ[哀れむ:가엾이 여기다:pity,哀れみ:가엾음:pity]|'
 *   '3|愛|love|*アイ[愛国:애국:patriotism,愛人:애인:lover]||'
 *
 * 제약사항:
...
1. **Enrich Kanji Meaning**: 
   - Add English meaning ('en_meaning') for the Kanji.
   - Format: 'id|kanji|emoji|en_meaning|...'
...
# Data Format Rules
- **Structure**: 'id|kanji|emoji|en_meaning|onyomi_readings|kunyomi_readings|unknown_readings'
...
- **Output**: 
'2|哀|😢|pity, sorrow|*アイ[哀悼:애도:condolence,哀愁:애수:melancholy,哀願:애원:entreaty,悲哀:비애:sorrow,哀愁:애수:melancholy,哀願:애원:entreaty,悲哀:비애:sorrow]|*あわれ[哀れ:비애:pity,哀れな話:가련한 이야기:pity story,哀れがる:가엾이 여기다:feel pity];あわれむ[哀れむ:가엾이 여기다:pity,哀れみ:가엾음:pity]|'
'3|愛|❤️|love|*アイ[愛国:애국:patriotism,愛人:애인:lover]||'
...
- Maintain the exact number of pipes ('|') in each line (now 7 fields = 6 pipes).
 ...
 * 형식: id|kanji|emoji|en_meaning|onyomi_readings|kunyomi_readings|unknown_readings
...
function parseCompressedData(compressedData) {
  ...
  return lines.map((line) => {
    ...
    if (parts.length < 7) {
      // 기존 형식 지원 (emoji가 없는 경우)
      if (parts.length === 6) {
        const [id, kanji, enMeaning, onyomiStr, kunyomiStr, unknownStr] = parts;
        return {
          id: parseInt(id, 10),
          kanji,
          emoji: "",
          meanings: { en: enMeaning || "" },
...
function mergeSingleItem(originalItem, llmItem, originalBatchItem) {
  ...
  return {
    ...result,
    meanings: {
      ...originalItem.meanings,
      en: enArray,
      ko: originalItem.meanings?.ko || "",
    },
